...
describe("Options", function() {
  ...
  describe("fromObjectWithPrefix", function() {
    it("should create a new backing object with just the keys containing the prefix", function() {
      var opts = Options.fromObjectWithPrefix({
      ...
      assert.equal(opts.getOrError("one"), "one");
      assert.equal(opts.getOrError("two"), "two");
      assert.equal(opts.getOrError("three"), "three");

      assert.deepEqual(opts.toObject(), {
        one: "one",
        two: "two",
        three: "three"
      });
